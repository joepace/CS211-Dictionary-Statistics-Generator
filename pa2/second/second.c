...
/*
The readDict() function uses a parameter using a pointer pointing to the dictionary file to read in words to add to the Trie structure.
*/
...
void matchStr(char* str){
	...
	while(pointer != NULL){
		...
		if((pointer = pointer->children[index]) == NULL){
			return;
		}
		...
		if(height == (length - 1) && pointer-> end == true){
...
/*
	The newTree() function creates a pointer to a new Trie tree or sub-tree, depending on what variable the new pointer is being saved to.

*/
struct TrieNode * newTree(){
	struct TrieNode * pointer = NULL;
	
	pointer = (struct TrieNode *) malloc(sizeof(struct TrieNode));
...
void insert(char * key){
	...
	for(height = 0; height < length; height++){
		...
		if(!traversal_pointer -> children[index]){
...
/*
	The freeAllNodes() function will recursively traverse the entire Trie tree and free each node.
*/
